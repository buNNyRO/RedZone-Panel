...
class this {
	...
	public static $_vehVipColors = array();
	private static $_perPage = 18;

	public static $_DOMAIN = 'red-zone.ro';
	public static $_API = 'dee643ca45e2af7ee2c13b7b7de4ea599cd731ca';
	...
	public static function getContent() {
		...
		include_once 'str/global/header.cw.php';
		if(self::$_url[0] === 'action') { include 'actions/' . self::$_url[1] . '.a.php'; return; }
		if(isset(self::$_url[0]) && !strlen(self::$_url[0]))
			include_once 'str/index.cw.php';
		else if(file_exists('str/' . self::$_url[0] . '.cw.php')) 
			include 'str/' . self::$_url[0] . '.cw.php';
		else
			include_once 'str/index.cw.php'; 
		include_once 'str/global/footer.cw.php';
	}
	...
	public static $_PAGE_URL = 'https://red-zone.ro/panel/';
	...
	public static function checkDon() {
		...
		if(strlen($data) > 15) { //DELETE AFTER DEMO
			...
			for($x = 0; $x < self::$_DONATION; $x++) {
				...
				$row = explode(",",$result);
				for($i = 0; $i < 4; $i++) {
					$w = connect::$g_con->prepare('SELECT `ID` FROM `panel_donations` WHERE `Username` = ? AND `CreatedAt` = ?');
					$w->execute(array($row[0],$row[3]));
					if(!$w->rowCount()) {
						$wcc = connect::$g_con->prepare('INSERT INTO `panel_donations` (`Username`,`Message`,`Amount`,`CreatedAt`) VALUES (?,?,?,?)');
	...
	private static function arrays() {
		...
		self::$_vehVipColors = array(
			'#FFFFFF', '#FF0000', '#640000', '#00FF00', '#004B01', '#008DFF', '#001AFF', '#FFFF00', '#FF6B00', '#EB00FF',
			'#590056', '#8A8A8A', '#000000', '#28B2E0', '#F20C9A'
		);
	}
